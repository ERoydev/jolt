use crate::{error::GuestError, exec_syscall_handler::EXEC_OVERRIDE};
use ckb_vm::{Bytes, DefaultMachineBuilder, SupportMachine};
...
impl VmExecutor {
    ...
    pub fn execute(&self) -> Result<(), GuestError> {
        if !self.script_elf.starts_with(&ELF_MAGIC) {
            return Err(GuestError::InvalidElfMagic);
        }

        let machine_trace_impls = ProtobufVmRunnerImpls::new_with_bytes(&self.trace_data)
            .ok_or(GuestError::TraceDecodingError())?;
        ...
        let machine_version = match self.script_version {
            ...
            _ => return Err(GuestError::UnsupportedScriptVersion(self.script_version)),
        };
        ...
        machine
            ...
            .map_err(|_| GuestError::ProgramLoadError())?;

        if let Err(e) = machine.run() {
            return Err(GuestError::VmExecutionError(format!("{}", e)));
        }
